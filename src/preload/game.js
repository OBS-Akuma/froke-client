...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const juiceDiscordButton = () => {
    ...
    discordBtn.onclick = () => {
      window.open("https://discord.gg/ak9JjymMV5", "_blank");
    };
  ...
  const handleServers = async () => {
    const mapImages = await fetch(
      "https://raw.githubusercontent.com/Dev-Zenos/KirkaSkins/main/maps/full_mapimages.json"
    ).then((res) => res.json());

    Object.keys(mapImages).forEach((item) => {
      if (!mapImages[item].includes("https")) {
        mapImages[item] =
          "https://raw.githubusercontent.com/Dev-Zenos/KirkaSkins/main" +
          mapImages[item];
      }
    });
    ...
    const interval = setInterval(() => {
      ...
      if (document.querySelector(".profile > .content")) {
        ...
        if (shortId && shortId === "NUGGET") {
          ...
          `;
          div.innerHTML = `
          <img src="https://cdn.discordapp.com/avatars/1415346323930218596/a4c23bd8abf1a2d9f50ece02560e6c24.png" style="height: 0.8rem; width: auto;" />
          <span style="font-size: 1rem; font-weight: 600; color: #fff;">Kinda just better Smdugy.store</span>
          `;
